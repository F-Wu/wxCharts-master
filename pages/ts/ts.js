...
var app = getApp();
var storage = require('../../utils/storage');//数据
var ts = require('../../utils/wxChart/time-sharing');//wxChart
...
Page({
    ...
    tabChart: function (e) {
        ...
        var type = e.target.dataset.type; //ts5
        var data = this.data[type];//ts5数据
        console.log(data)
        this.setData({
            tabName: type,
            stock: data.name,
    ...
    renderTs2: function (data) {
        ts2.metaData2(data, getOptionTimeSharing2());//数据转化
        ts2.draw();
    },
    clearTimer: function () {
        clearInterval(timer);
